...
DEF schemaname=TANEL

-- DROP TABLE &schemaname..workload_control;
-- DROP TRIGGER &schemaname..logon_trigger;
...
CREATE TABLE &schemaname..workload_control (
  ...
  , indexes_visible VARCHAR2(100) DEFAULT 'VISIBLE' NOT  NULL
  , CONSTRAINT pk_workload_control PRIMARY KEY (username, service_name, module, action, program, client_machine, client_osuser)
  , CONSTRAINT ck1_workload_control CHECK (indexes_visible IN ('VISIBLE','NOT VISIBLE'))
);

INSERT INTO &schemaname..workload_control (username, indexes_visible) VALUES ('TANEL', 'NOT VISIBLE'); 
COMMIT;

CREATE OR REPLACE TRIGGER &schemaname..logon_trigger
...
BEGIN
    ...
    LOOP -- this loop returns only 1 row
        ...
        AND indexes_visible = 'NOT VISIBLE';

        IF c > 0 
            THEN EXECUTE IMMEDIATE 'alter session set optimizer_use_invisible_indexes = false';
            ELSE EXECUTE IMMEDIATE 'alter session set optimizer_use_invisible_indexes = true';
        END IF; 
    END LOOP;
END;
/
